(() => {
    ...
    document.querySelectorAll("._select").forEach((function(customSellectBody) {
        ...
        document.addEventListener("click", (function(e) {
            if (e.target !== selectLangBtn) selectLangList.classList.remove("_list_visible");
        }));
        document.addEventListener("keydown", (function(e) {
            if ("Tab" === e.key || "Escape" === e.key) selectLangList.classList.remove("_list_visible");
        }));
    }));
    const newElement = document.querySelector(".header");
    const cloneElement = newElement.cloneNode(true);
    const cloneWrapper = document.querySelector(".header__wrapper");
    cloneElement.classList.add("header--fixed");
    cloneWrapper.append(cloneElement);
    $(window).scroll((function() {
        console.log($(window).scrollTop());
        if ($(window).scrollTop() > 250) cloneElement.classList.add("header--is-show"); else cloneElement.classList.remove("header--is-show");
    }));
    window["FLS"] = true;
